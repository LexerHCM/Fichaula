...
/** Seguimientos (no eliminados) de una clase, vía vista. */
...
/**
 * Soft delete de un seguimiento (marca eliminado=true).
 * RLS permite solo al autor; admin tiene lectura pero no borra aquí.
 */
async function eliminarSeguimientoDB(seguimientoId) {
  const user = getUsuarioActual();
  if (!user) return { ok: false, error: 'Necesitás iniciar sesión.' };
  try {
    await sb.patch(`seguimientos?id=eq.${encodeURIComponent(seguimientoId)}`,
      { eliminado: true });
    return { ok: true };
  } catch (e) {
    return { ok: false, error: _msg(e) };
...
// ╔══════════════════════════════════════════════════════════╗
// ║  COMENTARIOS (crear / editar / soft-delete) — Nivel 1    ║
// ╚══════════════════════════════════════════════════════════╝

/** Comentarios NO eliminados de un alumno. */
async function getComentariosDeAlumnoDB(alumnoId) {
  return sb.get(
    `comentarios?alumno_id=eq.${encodeURIComponent(alumnoId)}&eliminado=eq.false&select=*&order=creado_en.desc`);
}
...
/** SOFT DELETE: marca eliminado=true en vez de borrar la fila. */
async function eliminarComentarioDB(comentarioId) {
  const user = getUsuarioActual();
  if (!user) return { ok: false, error: 'Necesitás iniciar sesión.' };
  try {
    await sb.patch(`comentarios?id=eq.${encodeURIComponent(comentarioId)}`,
      { eliminado: true });
    return { ok: true };
  } catch (e) {
    return { ok: false, error: _msg(e) };
...
async function eliminarClaseDB(id) {
  try {
    await sb.delete(`clases?id=eq.${encodeURIComponent(id)}`);
    return { ok: true };
  } catch (e) {
    const m = _msg(e);
...
async function eliminarAlumnoDB(alumnoId) {
  try {
    await sb.delete(`alumnos?id=eq.${encodeURIComponent(alumnoId)}`);
    return { ok: true };
  } catch (e) {
    return { ok: false, error: _msg(e) };
  }
...
/** Todos los seguimientos (no eliminados) para la vista del admin. */
